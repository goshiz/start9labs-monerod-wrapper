...
fn sidecar(config: &Mapping, addr: &str) -> Result<(), Box<dyn Error>> {
    ...
    if let (Some(user), Some(pass)) = (
        ...
        stats.insert(
            ...
            Stat {
                ...
                description: Some("monero-Standup Quick Connect URL"),
                copyable: true,
                qr: true,
                masked: true,
            },
        ...
        stats.insert(
            ...
            Stat {
                value_type: "string",
                value: format!("{}", user),
                description: Some("Monero RPC Username"),
                copyable: true,
                masked: false,
                qr: false,
            },
        ...
        stats.insert(
            ...
            Stat {
                value_type: "string",
                value: format!("{}", pass),
                description: Some("Monero RPC Password"),
                copyable: true,
                masked: true,
                qr: false,
            },
    ...
    let info_res = std::process::Command::new("monero-wallet-cli")
    ...
    if info_res.status.success() {
        ...
        {
            std::process::Command::new("monero-wallet-cli")
                .arg("-conf=/root/.monero/monero.conf")
                .arg("pruneblockchain")
...
fn inner_main(reindex: bool) -> Result<(), Box<dyn Error>> {
    ...
    std::io::copy(
        &mut TemplatingReader::new(
            std::fs::File::open("/root/.monero/monero.conf.template")?,
            &config,
            &"{{var}}".parse()?,
            b'%',
        ),
